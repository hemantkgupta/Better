...
public class LL027SegregateEvensAndOddsInList {
	...
	public static void main(String[] args) {
			...
			basic.Node root = new basic.Node(12);
			root.next = new basic.Node(15);
			root.next.next = new basic.Node(10);
			root.next.next.next = new basic.Node(11);
			root.next.next.next.next = new basic.Node(5);
			root.next.next.next.next.next = new basic.Node(6);
			root.next.next.next.next.next.next = new basic.Node(2);
			root.next.next.next.next.next.next.next = new basic.Node(3);
			segregateEvenOdd(root);
			System.out.println(root);

	}

	public static void segregateEvenOdd(basic.Node head_ref) {
		basic.Node end = head_ref;

		basic.Node prev = null;
		basic.Node curr = head_ref;

		/* Get pointer to the last node */
		while (end.next != null)
			end = end.next;

		basic.Node new_end = end;
